...
const { validaContatos } = require('./midlewares')
...
router.post('/', validaContatos, async (req, res) => {
    try {
        let cliente = await pool.connect()
        let dados = await cliente.query('insert into tb_contatos(nome, email)values($1, $2) RETURNING *',
